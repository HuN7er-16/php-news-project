...
class Comment extends Admin{
    ...
    public function index(){
        ...
        $db = new DataBase();
        $comments = $db->select('SELECT comments.*, posts.title AS post_title, users.email AS email FROM comments LEFT JOIN posts ON comments.post_id = posts.id LEFT JOIN users ON comments.user_id = users.id ORDER BY `id` ASC');
